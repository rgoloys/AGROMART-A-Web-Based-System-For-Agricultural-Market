...
<div class="container">  
  ...
  <form action="<?= base_url('ordered/'. $product->id); ?>" method="POST" enctype="multipart/form-data" >
  ...
  <div class="DelieveryAddress" style="padding: 12px;box-shadow: 2px 2px 4px #008374;border-radius: 5px; margin:1.5% 1.5%;">
    ...
    <p class="snamess"><span>Product ID: <input type="text" name="p_id" value="<?= $product->id ?>"  readonly style="border:none;"></span> <span><img src="<?= base_url('/Impact/icons/icons8-shop-94.png'); ?>">Shop: <b><?= $products->sname; ?></b></span></p>    
       
    <div class="tblsss">
      <table class="porty">
        <tr>
          <td rowspan="3"><img src="<?= base_url('/upload/images/'. $product->image1) ?>"></td>
          <td style="color: #008374;"><?= $product->p_title; ?></td>
        </tr>
        <tr>
          <td>Price: ₱<?= $product->p_price; ?></td>
        </tr>
        <tr>
          <td>Pack size: ₱<?= $product->p_size; ?></td>
        <?php
if ($product->dnum != 0) {
    echo '<tr>';
    echo '<td style="color:green;"><b>Buy ' . $product->dnum . ' get less ₱' . $product->dprice . ' for each product</b></td>';
    echo '</tr>';
}
?>
   ...
   <table class="sixty">
    ...
    <tr>
        <td>Shipping fee: </td>
        <td>₱<?= $product->p_shipping; ?></td>
    </tr>
    <tr id="discount" style="display:none;">
        <td>Discount: </td>
        <td colspan="2"><span id="numdis"></span></td>
    </tr>
    <tr>
        <td>Select quantity: </td>
        <td class="qtyy" style="cursor:pointer;display:flex; justify-content:start; width: 100%;">
            <span onclick="updateQuantity('sub')"> -</span>
            <input type="number" class="qty" name="Quantity" value="1" min="1" max="<?= $product->p_stocks; ?>" readonly style="width:30px; text-align: center; border:none;">
            <span onclick="updateQuantity('add')">+</span>
        </td>
    </tr>
    <tr>
        ...
        value="<?php if($product->p_shipping == 50){echo $product->p_price + $product->p_shipping;}else{echo $product->p_price;}  ?>" readonly></h6>
        </td>
    </tr>
</table>
        ...
        <div id="output">   
          ...
          <div id="imageContainer1" style="display: none; justify-content:center; text-align:center;">
            ...
            <h6>AGROCASH</h6>
            <ul>
              <li>Available balance: <?= $bal['am']; ?>.00</li>
              <li>Total purchase: ₱<input class="ttls" type="number" name="Total_Price" id="PPP"  
        value="<?php if($product->p_shipping == 50){echo $product->p_price + $product->p_shipping;}else{echo $product->p_price;}  ?>" readonly></li>
              <li>Remaining balance: ₱<input class="ttls" type="number" name="Remaining" id="TTT"  
        value="" readonly></li>
            </ul>
            <p class="ckout"><button type="submit" id="submit">Checkout</button></p>
          </div>
...
<script>
    ...
    function calculateTotal() {
        ...
        var dnum = parseFloat('<?= $product->dnum; ?>');
        var dprice = parseFloat('<?= $product->dprice; ?>');
        var shipping = <?= $product->p_shipping; ?>;
        var totall =   shipping + (price * quantity );


        if (dnum <= quantity) {
            var dis = quantity * dprice;
            document.getElementById('discount').style.display = 'table-row';
            document.getElementById('numdis').innerHTML = dis.toFixed(2);
        } else {
            var dis = 0;
        ...
        if(agro >= 0){
          document.getElementById('PPP').value = total.toFixed(2);
          document.getElementById('TTT').value = agro.toFixed(2);
        }else{
